...
function keyword_spaces() # verifie le espaces pres les mots cles (return | if | while)
{
    ...
    while read VAR1; do
    	...
    	print_error $1 "ligne $N3VAR: pas d'espace aprÃ¨s mot clef"
    	NB_ERROR=$((NB_ERROR + 1))
    done < $TMP5
}
...
function nb_col()
{
	...
	if [[ $NB_CARAC -gt 81 ]]; then
	    print_error $1 "ligne $INDEX $NB_CARAC caractÃ¨res"
	    NB_ERROR=$(($NB_ERROR + 1))
	fi
	INDEX=$(($INDEX + 1))
    done
...
function check_include() # verifie l'ordres des  #includes
{
	...
	print_error $1 "include systeme mal placÃ©"
	NB_ERROR=$(($NB_ERROR + 1))
    fi
}
...
function check_declaration() # verifie les declarations / affectations
{
	...
	if [[ `echo $LINE | wc -w` > 2 ]]; then
	    print_error $1 "ligne `echo $LINE | tr ":" " "| awk '{print $1}'` dÃ©claration et affectation Ã  la mÃªme ligne"
	NB_ERROR=$((NB_ERROR + 1))
	fi
    done < $TMP8
}
...
function double_line() # verifie les sauts de ligne inutiles
{
    ...
    do
	print_error $1 "ligne $INDEX double retour Ã  la ligne"
	INDEX=$(($INDEX + 1))
	NB_ERROR=$((NB_ERROR + 1))
    done
}
...
function check_malloc() # verifie les mallocs
{
	...
	for NB in $NBL
	do
	    print_error $1 "ligne $NB, malloc non verifiÃ©"
	    NB_ERROR=$((NB_ERROR + 1))
	done
    fi
}
...
function presentation() # affiche le message d'info
{
    ...
    echo -e "prÃ©sence d'un Makefile et d'un executable.\n"
    echo -e "GÃ©rÃ©:"
    echo -e " âžœ fonctions de plus de 25 lignes"
    echo -e " âžœ mauvais header"
    echo -e " âžœ fonctions autorisÃ©es/interdites"
    echo -e " âžœ nombre de fonctions par fichier"
    echo -e " âžœ espace manquant aprÃ¨s mot clef"
    echo -e " âžœ double retour Ã  la ligne"
    echo -e " âžœ ligne de plus de 80 caractÃ¨res"
    echo -e " âžœ plus de 4 paramÃ¨tres pour une fonction"
    echo -e " âžœ commentaire dans le code"
    echo -e " âžœ ordonnencement des include"
    echo -e " âžœ declaration et affectation sur la mÃªme ligne"
    echo -e " âžœ saut de ligne aprÃ¨s les dÃ©clarations"
    echo -e " âžœ protection headers"
    echo -e " âžœ multi-instruction"
    echo -e " âžœ define dans un .c"
    echo -e " âžœ norme Makefile"
    echo -e " âžœ espace en fin de ligne"
    echo -e " âžœ nom des variables et structures"
    echo -e " âžœ tabulation dans les dÃ©clarations"
    echo -e " âžœ retour a la ligne manquant entre deux fonctions"
    echo -e " âžœ saut de ligne aprÃ¨s les dÃ©clarations\n"
    echo -e "Sauvergarde les fonctions autorisÃ©es dans le repertoire:"
...
function usage() # affiche le usage
{
    ...
    echo -e "-R	--reset		oublier les fonctions autorisÃ©es"
    exit 0
}
